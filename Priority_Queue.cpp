...
int PQ_Refresh(PriorityQueue* PQ, PQNode* tNode)   // 1 : 새로운 값으로 바뀜, 0 : 존재하는 값 없음, -1 : 새로운 값으로 바뀌지 않음
{
	int i;
	for(i=0;i<PQ->UsedSize;i++)
	{
		if(PQ->Nodes[i].Data->first==tNode->Data->first && PQ->Nodes[i].Data->second==tNode->Data->second)
		{
			if(PQ->Nodes[i].Priority>tNode->Priority)
			{
				PQ->Nodes[i].Priority=tNode->Priority;
				return 1;
			}
			else
				return -1;
